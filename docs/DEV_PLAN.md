...
- **Código**: `install.ps1` e `uninstall.ps1` na raiz do projeto (não um `npm run setup`, para não precisar de `node_modules` já instalado só para rodar o instalador). `scripts/bootstrap-pastas.mjs` novo, reutiliza `garantirEstruturaDePastas` (mesma função usada no boot normal desde a Etapa 1) em vez de duplicar a lógica de criação de pastas em PowerShell.
- **Gap descoberto nesta etapa, não previsto no PRD/DEV_PLAN original**: em nenhum lugar do código existia uma chamada para *criar* a instância `Q-Zap` na Evolution API — `aguardarConexao` só consulta `GET /instance/connectionState/Q-Zap`, que falha se a instância não existir. Nas Etapas 1-10 isso sempre foi feito manualmente pelo usuário via manager. Investigado lendo o bundle do container (mesmo método das Etapas 4 e 7): `POST /instance/create` (`instanceName`, `qrcode`, `Integration`), `GET /instance/connect/:instanceName` (dispara a geração do QR, usado internamente pelo manager), `GET /instance/fetchInstances` (lista instâncias existentes, usado para idempotência em reinstalação). Decisão questionada com o usuário antes de aplicar (CLAUDE.md exige isso, via `AskUserQuestion`): `install.ps1` cria a instância via API automaticamente (se ainda não existir) e **aguarda o pareamento** (poll de `connectionState` a cada 3s, até 5min) antes de registrar o serviço do Windows — evita o cenário alternativo (registrar e iniciar o serviço sem esperar) em que o serviço entraria em crash-loop até esgotar as 3 tentativas de restart do `node-windows` (Etapa 10) e parar sozinho, exigindo reinício manual depois do QR escaneado. Em vez de renderizar o QR no terminal, o script abre `http://localhost:8080/manager` no navegador padrão (`Start-Process`) — reaproveita a UI que já mostra o QR de forma legível, sem lógica nova de renderização.
- **Detecção automática do comando `docker compose`** (questionado com o usuário antes de aplicar): tenta `docker compose` (plugin v2, padrão atual do Docker Desktop) e cai para `docker-compose` (binário standalone legado) se o primeiro não estiver disponível — mais robusto que fixar uma variante só, dado que a instalação roda em máquinas de cliente com Docker de proveniências variadas.
- **Elevação (Administrador)** feita via `#Requires -RunAsAdministrator` (mecanismo nativo do PowerShell) em vez de checagem manual — se o script não estiver rodando elevado, o próprio PowerShell recusa a execução com uma mensagem padrão pedindo para reabrir como Administrador. Decisão questionada com o usuário antes de aplicar (CLAUDE.md exige isso) contra auto-elevação (`Start-Process -Verb RunAs`); a checagem simples venceu por não abrir uma janela nova nem quebrar a continuidade do console.
- **`LIMITE_DIARIO_POR_NUMERO` sugerido como padrão na pergunta interativa**: `50` mensagens/dia (aceitável com Enter) — valor escolhido com o usuário entre opções mais conservadoras (20) e mais permissivas (100); guideline anti-ban só recomenda "começar conservador" sem número fixo.
- **`EVOLUTION_API_KEY`/`POSTGRES_PASSWORD` gerados nativamente em PowerShell**: 32 bytes aleatórios via `System.Security.Cryptography.RandomNumberGenerator`, convertidos para hex (64 caracteres) — sem dependência de Python (RNF08) nem de módulos externos do PowerShell. Só gera se a variável já não estiver preenchida no `.env` (idempotente em reexecuções, não invalida uma instalação/pareamento já feitos).
- **Escrita do `.env`** via `[System.IO.File]::WriteAllLines` com `UTF8Encoding($false)` (sem BOM) — testado isoladamente nesta etapa; um BOM no início do arquivo quebraria a leitura da primeira variável pelo `dotenv` (`PASTA_BASE` é a primeira linha do `.env.example`).
- **Confirmação recusada na pergunta de resumo**: `.env` recém-criado é apagado (`Remove-Item`) e o script aborta, para que uma reexecução dispare de novo o fluxo de primeira instalação (sem isso, a próxima execução veria um `.env` já existente e pularia as perguntas, deixando `PASTA_BASE` etc. vazios permanentemente).
- **Ainda não validado nesta sessão**: execução real de ponta a ponta em máquina limpa/VM (`install.ps1` → `uninstall.ps1` → reinstalar), conforme os dois últimos bullets desta etapa no plano original. O que foi validado: sintaxe de ambos os scripts (parser do PowerShell, sem erros), as funções de leitura/escrita do `.env` e geração de token (testadas isoladamente contra uma cópia descartável do `.env.example`) e `scripts/bootstrap-pastas.mjs` (rodado de verdade contra o `.env` de teste do projeto, pastas criadas corretamente). PRD.md seção 11 mantém os 2 itens de instalação/desinstalação como `[ ]` até essa validação de ponta a ponta acontecer.
- **Auto-instalação do Node.js via `winget`** (refinamento pedido pelo usuário após a etapa fechada, discutido antes de aplicar): `install.ps1` agora tenta `winget install --id OpenJS.NodeJS.LTS` quando `node` não é encontrado, e atualiza o `PATH` da sessão atual lendo direto do registro (`[System.Environment]::GetEnvironmentVariable('Path', 'Machine'/'User')`) para não precisar reabrir o PowerShell no meio da instalação. Cai para a instrução manual (link para nodejs.org) se o `winget` não existir na máquina ou a instalação falhar. **Docker Desktop ficou de fora da auto-instalação, decisão explícita**: costuma exigir habilitar WSL2/Hyper-V com reinício da máquina no meio do processo (o script não teria como retomar sozinho depois de um reboot) e tem tela própria de aceite de termos; além disso, o Docker Desktop exige assinatura paga para empresas grandes (acima de USD 10M de receita ou 250+ funcionários) — instalar isso silenciosamente no ambiente de um cliente sem ele saber é um risco de compliance que não cabe a um script de instalação decidir sozinho. A mensagem de erro de Docker ausente foi melhorada (link direto + aviso sobre WSL2/reinício) em vez de auto-instalar.
